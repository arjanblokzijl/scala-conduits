...
//  def register[A]: ResourceT[F, A] = ResourceT(istate =>
//      resourceLiftBase(istate)
//  )
  ...
  //      , ReleaseKey key
  //      )
//  def registerRef[F[_], A](istate: HasRef#Ref[F, ReleaseMap[A]], rel: F[Unit]): F[ReleaseKey] = {
//    hasRef.atomicModifyRef(istate)(rMap => )
//  }
}
object resources {
...
object ReleaseMap {
  def apply[A](base: A): ReleaseMap[A] = ReleaseMap(base, 0, 0)
}
case class ReleaseMap[A](base: A, nextKey: Int, refCount: Int, m: Map[Int, A] = Map[Int, A]())
//
...
//  def value:
//}
case class ResourceT[F[_], A](trans: (HasRef[F]#Ref[F[_], A], ReleaseMap[A]) => F[A])
