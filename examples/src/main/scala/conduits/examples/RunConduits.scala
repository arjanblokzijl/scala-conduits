...
object RunConduits extends App {
  ...
  val sinkTakeM = CL.takeM[Stream, Int](10)
  val sinkTake = CL.take[IO, Int](10)
  val sinkConsume = CL.consumeM[Stream, Int]
  ...
  println("result takeM stream " + (sourceStream >>== sinkTakeM).flatten.take(15).force)
  println("result takeM id " + (sourceId >>== CL.takeM[Id, Int](10)))//this sums the stream, so that makes the whole approach a bit strange
