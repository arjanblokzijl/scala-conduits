...
//Copy of Akka's ByteIterator implementation, used for benchmarking.
...
object ByteIterator {
  object ByteArrayIterator {
    ...
    def apply(array: Array[Byte]): ByteArrayIterator =
      new ByteArrayIterator(array, 0, array.length)

    def apply(array: Array[Byte], from: Int, until: Int): ByteArrayIterator =
  ...
  class ByteArrayIterator private (private var array: Array[Byte], private var from: Int, private var until: Int) extends ByteIterator {
    ...
    final override def toAkkaByteString: AkkaByteString = {
      val result =
        if ((from == 0) && (until == array.length)) AkkaByteString.AkkaByteString1C(array)
        else AkkaByteString.AkkaByteString1(array, from, len)
      clear()
      result
    }
  ...
  object MultiByteArrayIterator {
    ...
    def apply(iterators: LinearSeq[ByteArrayIterator]): MultiByteArrayIterator =
  ...
  class MultiByteArrayIterator private (private var iterators: LinearSeq[ByteArrayIterator]) extends ByteIterator {
    ...
    final override def toAkkaByteString: AkkaByteString = {
      if (iterators.tail isEmpty) iterators.head.toAkkaByteString
      else {
        val result = iterators.foldLeft(AkkaByteString.empty) { _ ++ _.toAkkaByteString }
        clear()
        result
      }
    }
...
/**
 * An iterator over a AkkaByteString.
 */
...
abstract class ByteIterator extends BufferedIterator[Byte] {
  ...
  def toAkkaByteString: AkkaByteString

  override def toSeq: AkkaByteString = toAkkaByteString
